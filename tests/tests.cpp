...
#define RAW_SMART_PTR_DEBUG

#define SMART_PTR_DEBUG_PRINT(stream_expression) \
	do {                                         \
		std::cout << stream_expression;          \
	} while (0)

#include "classes/unique_ptr/unqiue_ptr.h"
#include "raw_namespace_functions.h"

static int s_active_test_objects = 0;
...
public:
	...
	explicit TestObject(int _id = 0) : id(_id) {
		...
		SMART_PTR_DEBUG_PRINT("TestObject " << id << " constructed. Active objects: "
											<< s_active_test_objects << std::endl);
	}

	~TestObject() {
		s_active_test_objects--;
		SMART_PTR_DEBUG_PRINT("TestObject " << id << " destructed. Active objects: "
											<< s_active_test_objects << std::endl);
	}

	TestObject(TestObject&& other) noexcept : id(other.id) {
		SMART_PTR_DEBUG_PRINT("TestObject " << id << " moved." << std::endl);
		other.id = -1;
	}

	TestObject& operator=(TestObject&& other) noexcept {
		id = other.id;
		SMART_PTR_DEBUG_PRINT("TestObject " << id << " move-assigned." << std::endl);
		other.id = -1;
		return *this;
	}
};
...
void verify_active_objects(const std::string& test_name, int expected_count) {
	std::cout << "--- VERIFY: " << test_name << " --- ";
	if (s_active_test_objects == expected_count) {
		std::cout << "SUCCESS! Active objects: " << s_active_test_objects
				  << " (Expected: " << expected_count << ")\n";
	} else {
		std::cout << "FAILED! Active objects: " << s_active_test_objects
				  << " (Expected: " << expected_count << ")\n";
...
void stress_test_unique_ptr(int iterations, int max_pointers_in_pool = 100) {
	...
	std::vector<size_t> array_sizes(max_pointers_in_pool, 0);

	int current_single_objects = 0;
	int current_array_objects  = 0;
	...
	for (int i = 0; i < iterations; ++i) {
		...
		try {
			if (op < 3) {
				...
				if (op == 3) {
					if (array_ptrs[idx1]) {
						current_array_objects -= array_sizes[idx1];
					}
					size_t new_size	  = dist_array_size(gen);
					array_ptrs[idx1]  = raw::make_unique<TestObject[]>(new_size);
					array_sizes[idx1] = new_size;
					current_array_objects += new_size;
					if (array_ptrs[idx1]) {
					...
					if (array_ptrs[idx2]) {
						current_array_objects -= array_sizes[idx2];
					}
					array_ptrs[idx1]	  = std::move(array_ptrs[idx2]);
					array_sizes[idx1]	  = array_sizes[idx2];
					array_sizes[idx2]	  = 0;
					current_array_objects = s_active_test_objects;
				} else if (op == 5) {
					if (array_ptrs[idx1] && array_sizes[idx1] > 0) {
						size_t elem_idx =
							std::uniform_int_distribution<>(0, (int)array_sizes[idx1] - 1)(gen);
			...
			if (i % 1000 == 0) {
				assert(s_active_test_objects >= 0 &&
					   s_active_test_objects <= max_pointers_in_pool * 20);
			}
...
int main() {
	...
	std::cout << "\nAll unique_ptr tests finished.\n";
	verify_active_objects("Final check after all tests", 0);
